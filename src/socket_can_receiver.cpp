...
bool SocketCANReceiver::bind(uint32_t frame_id) {
    ...
    ioctl(_socket, SIOCGIFINDEX, &_ifreq);

    //bind
    RCLCPP_DEBUG(rclcpp::get_logger("socket_can_receiver"), "[interface %s id 0x%X] binding", this->_ifname.c_str(),
                 frame_id);
    struct sockaddr_can _sockaddr_can{};
    _sockaddr_can.can_family = AF_CAN;
    _sockaddr_can.can_ifindex = _ifreq.ifr_ifindex;
    if (::bind(_socket, (struct sockaddr *) &_sockaddr_can, sizeof(_sockaddr_can)) != 0) {
        RCLCPP_DEBUG(rclcpp::get_logger("socket_can_receiver"),
                     "[interface %s id 0x%X errno %d] failed to bind to interface", this->_ifname.c_str(), frame_id,
                     errno);
        return false;
    }
    ...
    if (setsockopt(_socket, SOL_CAN_RAW, CAN_RAW_FILTER, &_can_filter, sizeof(_can_filter)) != 0) {
        RCLCPP_DEBUG(rclcpp::get_logger("socket_can_receiver"),
                     "[interface %s id 0x%X errno %d] failed to set can filter", this->_ifname.c_str(), frame_id,
                     errno);
        return false;
    }
